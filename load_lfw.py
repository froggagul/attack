...
def load_lfw_attr(attr='gender'):
    if attr in BINARY_ATTRS:
        return load_lfw_binary_attr(attr)    
    elif attr in MULTI_ATTRS:
        return load_lfw_multi_attr(attr)
    else:
        raise ValueError(f"{attr} is not known attribute.Known attributes are {list(BINARY_ATTRS.keys()) + list(MULTI_ATTRS.keys())}")
...
def load_lfw_with_attrs(attr1='gender', attr2=None):
    ...
    # using only data having label 1 and label 2
